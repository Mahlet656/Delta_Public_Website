...
export const About: React.FC<AboutProps> = ({ setActivePage, lang }) => {
  ...
  <div className="absolute -bottom-5 -left-5 bg-white p-4 rounded-xl shadow-xl border border-slate-200 flex items-center gap-3 hidden sm:flex">
    ...
    <div>
      <p className="font-bold text-slate-900 text-xs">{t.licensedBadge || "Ministry License #4812"}</p>
      <p className="text-[11px] text-slate-500">{t.officiallyApprovedAgency || "Officially Approved Hajj & Umrah Agency"}</p>
    </div>
  </div>
</div>
      </section>
