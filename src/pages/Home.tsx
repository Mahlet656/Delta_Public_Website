...
export const Home: React.FC<HomeProps> = ({
    ...
    <div className="space-y-12 pb-12 bg-[#F9F9F9] text-slate-800">
      ...
      <section className="relative min-h-[520px] lg:min-h-[580px] flex items-center overflow-hidden bg-slate-900">
        ...
        <div 
          ...
          style={{ backgroundImage: `url('/background/bg1.jpg')` }}
        />
        
        {/* Gradient Overlay - Cleaner and more modern */}
        <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/50 to-black/20" />
        
        {/* Decorative overlay pattern */}
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-0 right-0 w-96 h-96 bg-[#C8102E] rounded-full blur-3xl" />
          <div className="absolute bottom-0 left-0 w-72 h-72 bg-[#C8102E] rounded-full blur-3xl" />
        </div>

        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-8 py-16 w-full">
          <div className="max-w-2xl space-y-5">
            {/* Badge */}
            <span className="inline-flex items-center gap-2 bg-[#C8102E]/20 backdrop-blur-sm border border-[#C8102E]/30 text-[#C8102E] text-xs font-extrabold tracking-widest uppercase px-4 py-1.5 rounded-full">
              <span className="w-2 h-2 bg-[#C8102E] rounded-full animate-pulse" />
              {t.licensedBadge}
            </span>

            {/* Title with text shadow for readability */}
            <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black text-white leading-tight drop-shadow-lg">
              {t.heroTitle}
            </h1>
            
            {/* Subtitle */}
            <p className="text-sm sm:text-base text-white/90 leading-relaxed font-normal drop-shadow-md max-w-lg">
              {t.heroSubTitle}
            </p>
            ...
            <div className="pt-2 flex flex-wrap items-center gap-3">
              <button
                ...
                className="bg-[#C8102E] hover:bg-[#a60d25] text-white font-bold text-xs sm:text-sm px-6 py-3.5 rounded-lg shadow-lg transition-all flex items-center gap-2 hover:scale-105 transform duration-200"
              >
              ...
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs sm:text-sm px-5 py-3.5 rounded-lg shadow-lg transition-all flex items-center gap-2 border border-emerald-700 hover:scale-105 transform duration-200"
              >
                <svg 
            ...
            <div className="pt-4 flex items-center gap-6 text-white/70">
              <div className="flex items-center gap-2">
                <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse" />
                <span className="text-xs font-medium">{t.trustedBy}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[#C8102E] text-sm font-bold">★</span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 -mt-10 sm:-mt-14 relative z-20">
        <div className="bg-white rounded-xl shadow-xl border border-slate-200/80 p-4 sm:p-6 grid grid-cols-2 md:grid-cols-4 gap-4 divide-y sm:divide-y-0 sm:divide-x rtl:sm:divide-x-reverse divide-slate-100">
          ...
          <div className="flex items-center gap-3 p-2">
            ...
            <div>
              <h4 className="font-bold text-xs text-slate-900">{t.bestPackageLabel}</h4>
              <p className="text-[10px] text-slate-500">{t.bestPackagesShort}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 p-2 pt-3 sm:pt-2 sm:pl-4">
            ...
            <div>
              <h4 className="font-bold text-xs text-slate-900">{t.flightsLabel}</h4>
              <p className="text-[10px] text-slate-500">{t.flightsQuick}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 p-2 pt-3 sm:pt-2 sm:pl-4">
            ...
            <div>
              <h4 className="font-bold text-xs text-slate-900">{t.supportLabel}</h4>
              <p className="text-[10px] text-slate-500">{t.supportQuick}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 p-2 pt-3 sm:pt-2 sm:pl-4">
            ...
            <div>
              <h4 className="font-bold text-xs text-slate-900">{t.trustedAgencyLabel}</h4>
              <p className="text-[10px] text-slate-500">{t.trustedQuick}</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            <div>
              <span className="text-xs font-bold text-[#C8102E] uppercase tracking-wider flex items-center gap-1">
                <Sparkles className="w-3.5 h-3.5" /> {t.flightPartnersLabel}
              </span>
          ...
          <div className="relative overflow-hidden">
            <div className="flex animate-scroll gap-6">
              {/* First set of airlines */}
              {airlines.map((airline, index) => (
                <div 
                  key={`first-${index}`}
                  className="relative overflow-hidden rounded-2xl p-6 shadow-md border border-slate-200 flex-shrink-0 w-72 min-h-[280px] group"
                >
                  <div 
                    className="absolute inset-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-105"
                    style={{ backgroundImage: `url(${airline.image})` }}
                  />
                  <div className="relative z-10 space-y-3 flex-1 flex flex-col justify-between h-full">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className="text-3xl">{airline.flag}</span>
                        <div>
                          <h3 className="font-extrabold text-black text-xl drop-shadow-lg">{airline.name}</h3>
                          <span className="text-sm text-black font-semibold drop-shadow-lg">{airline.code}</span>
                        </div>
                      </div>
                    </div>
                    <div className="space-y-2 text-sm text-white bg-black/40 backdrop-blur-sm p-4 rounded-xl border border-white/20">
                      ...
                      <p><strong className="text-white">Baggage:</strong> {airline.baggage}</p>
                      <p><strong className="text-white">In-Flight:</strong> {airline.inflight}</p>
                    </div>
                    <div className="pt-2 border-t border-white/20 flex items-center justify-between">
                      <span className="font-bold text-white text-sm drop-shadow-lg">{airline.frequency}</span>
                      <span className="text-sm text-white italic drop-shadow-lg">{airline.badge}</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6 border border-slate-200 shadow-sm">
            <div className="space-y-1 text-center md:text-left rtl:md:text-right">
              <span className="text-xs font-bold text-[#C8102E] uppercase tracking-wider flex items-center gap-1 justify-center md:justify-start">
                <Sparkles className="w-3.5 h-3.5 text-[#C8102E]" /> {t.directAssistance}
              </span>
              <h3 className="text-lg sm:text-xl font-extrabold text-slate-900">{t.customFlight}</h3>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200 space-y-3 hover:shadow-md transition-shadow">
            ...
            <h3 className="font-bold text-slate-900 text-sm">{t.ourLocation}</h3>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200 space-y-3 hover:shadow-md transition-shadow">
            ...
            <h3 className="font-bold text-slate-900 text-sm">{t.officeHours}</h3>
            <p className="text-sm text-slate-600">
              <span className="font-semibold">{t.monToSatLabel || "Monday – Saturday:"}</span> {t.monToSatTime || "2:30 AM – 11:30 PM LT"}
              <br />
              <span className="font-semibold">{t.sunAndHolidaysLabel || "Sunday & Holidays:"}</span> {t.sunAndHolidaysDesc || "On-call WhatsApp Assistance"}
            </p>
          </div>
  ...
  <h3 className="font-bold text-slate-900 text-sm">{t.contactUs}</h3>
    ...
    <p className="flex items-center gap-2 pt-1 border-t border-slate-100">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-8 pt-4">
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-md p-6 sm:p-8">
          <div className="flex items-center justify-between border-b border-slate-200 pb-4 mb-4">
            <div>
              <span className="text-xs font-bold text-[#C8102E] uppercase tracking-wider">{t.recentGallery}</span>
              <h2 className="text-xl font-black text-slate-900 mt-0.5">{t.recentGalleryTitle}</h2>
            </div>
            <button
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-8 pt-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {currentReview && (
              <>
                <p className="text-xs text-slate-600 italic leading-relaxed">
                  "{(lang || '').toUpperCase() === 'AR' 
                    ? (currentReview.textAr || currentReview.text) 
                    : (lang || '').toUpperCase() === 'AM' 
                    ? (currentReview.textAm || currentReview.text) 
                    : currentReview.text}"
                </p>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-md p-6 space-y-4">
            <h3 className="font-bold text-sm text-slate-900">{t.ourAchievements}</h3>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500">{t.successfulRounds}</p>
                </div>
              </div>
            </div>
